...
export const deleteTweetRepo = async (
    tweetId: string
    ): Promise<{status: boolean, adminId: string | any}> => {
    try {
        const tweetBody = await TweetModel.findOneAndDelete({tweetId: tweetId});
        return {status: true, adminId: tweetBody?.adminId};
    } catch (error) {
        console.log(error);
